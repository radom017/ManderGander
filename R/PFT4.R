...
## Temperature = ambient temperature (degrees Celsius), Elev = elevation (m), RelativeHumidity = relative humidity (expressed as decimal), R = resistance to water loss (s/cm), mass = body mass (g), WTL = water time limit (decimal), WS = wind speed (m/s), BodyLength = length of body (m), Type = either 'T_eh', 'T_e', or 'T_a' (see function 'rho' for explanation), rehy = rate of rehydration (g/h), CTmax = x-intercept of energy assimilation curve (degrees Celsius), Topt = temperature where energy assimilation is maximized (degrees Celsius) , max_energy = energy from food at Topt (kJ), RMR_I = intercept of metabolic rate curve (log10(kJ)), RMR_T_s = slope of temperature in metabolic rate curve (unitless), RMR_M_s = slope of log10(mass) in metabolic rate curve (unitless), soil_temp = temperature of soil at depth where salamanders would remain if belowground (degrees Celsius).

PFT4=function(Temperature,Elev,RelativeHumidity,R,mass,WTL,WS,BodyLength,Type,rehy,CTmax,Topt,max_energy,RMR_I,RMR_T_s,RMR_M_s,soil_temp){
